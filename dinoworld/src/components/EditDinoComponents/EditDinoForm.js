...
import useFormEditDino from "./useFormEditDino";
...
export default function AddDinoForm({ submitForm }) {
  const {
    ...
    handleSubmit,
    errors
  } = useFormEditDino(submitForm, validateForm);

  return (
    <div className="form-content">
      <form action="" onSubmit={handleSubmit}>
        <h1>Let's make some changes with this dino</h1>
        ...
        <div className="form-inputs">
          ...
          <select
            ...
            name="top"
            className="input-field"
            value={values.top * 1} //Convert a boolean to a number (1 or 0)
            onChange={handleChange}
          >
        ...
        <button type="submit">Edit dinosaur</button>
      </form>
    </div>
  );
}
